...
async function insertData(username: string, email: string, password: string): Promise<void> {
  try {
...
async function insertAddressData(userId: number, city: string, country: string, street: string, pincode: string | null): Promise<void> {
  try {
    const result = await client.query(
      `
...
// Demo Transaction
async function demoTransaction(): Promise<void> {
  ...
  try {
    ...
    const resultUser = await clientTransaction.query(
      ...
      RETURNING id;
      `,
      ["mary_jones", "mary.jones@example.com", await bcrypt.hash("marypassword", 10)]
    );
